...
contract('TokenManager', ([owner, customer]) => {
  ...
  describe('#receiveFrom', () => {
    ...
    context('when enough amount is approved', () => {
      ...
      it('reduces tokens from customer balance', async () => {
        expect(await asset.balanceOf(customer)).to.be.bignumber.equal(
      ...
      it('transfers tokens to protocol balance', async () => {
        expect(
          await asset.balanceOf(tokenManager.address),
    ...
    context('when not enough amount is approved', () => {
      before(async () => {
        await asset.approve(tokenManager.address, amount.sub(toFixedBN(1)), {
          from: customer,
        });
      });
  ...
  describe('#sendTo', () => {
    ...
    context('when protocol balance is not enough', () => {
      before(async () => {
        asset = await createERC20Token(
          tokenManager.address,
          amount.sub(toFixedBN(1)),
        );
      });
