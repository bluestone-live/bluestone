...
describe('script: disableLoanAssetPair', () => {
  ...
  contract('LoanManager', () => {
    context('when the deposit is not enabled', () => {
      it('fails', async () => {
        const res = await disableLoanAssetPair(
          cb,
          network,
          loanTokenSymbol,
    ...
    context('when the deposit is enabled', () => {
      before(async () => {
        const [loanAsset, collateralAsset] = await enableLoanAssetPair(
          cb,
          network,
          loanTokenSymbol,
          collateralTokenSymbol,
        ...
        expect(
          await loanManager.isLoanAssetPairEnabled(loanAsset, collateralAsset),
        ).to.be.true;
      });

      it('succeeds', async () => {
        const [loanAsset, collateralAsset] = await disableLoanAssetPair(
          cb,
          network,
          loanTokenSymbol,
          collateralTokenSymbol,
