...
describe('script: setLiquidationDiscount', () => {
  ...
  contract('Configuration', () => {
    context('when the liquidation discount is valid', () => {
      ...
      it('succeeds', async () => {
        const [loanAsset, collateralAsset] = await setLiquidationDiscount(cb, network, loanTokenSymbol, collateralTokenSymbol, liquidationDiscount)
    ...
    context('when the liquidation discount is invalid', () => {
      ...
      it('fails', async () => {
        const res = await setLiquidationDiscount(cb, network, loanTokenSymbol, collateralTokenSymbol, liquidationDiscount)
